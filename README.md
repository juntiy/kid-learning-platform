...
> 💡 这是一个 **单文件、零依赖、纯前端** 的可交互原型，用浏览器直接打开 `index.html` 即可体验，所有状态用 `localStorage` 持久化（刷新不丢）。现已附带 **Vite 工程壳**（`npm run dev` 启动本地热更新服务器、`npm run build` 打包），方便后续迭代与部署；真正的数据隔离、SRS 调度引擎、家长登录鉴权等需要后端 + 数据库落地。
...
| 🔤 **英语字母** | 26 个字母大小写 + 音标 + 卡通图案 + 单词（点击听英文发音）；常用句型公式可造句；**「听音选词」小练习**（听发音选单词，答对 +5 积分、连对计火焰 🔥）。 |
| 🧠 **思维训练** | 找规律 / 图形逻辑闯关（难度缓慢递增、末段多组图案 6 选 1）+ 记忆翻牌游戏。 |
| 🔡 **拼音认识** | 单韵母 / 声母 / 复韵母，带 **四声**；专用中文发音（与英文字母名彻底分开）+ 拼读「拼一拼」演示 + **基础句型跟读**（带调短句逐字跟读）。 |
...
- **前端**：原生 HTML + CSS + JavaScript（`index.html` 单文件，内联样式与脚本，零运行时依赖）；以 **Vite** 作为开发 / 构建壳（`npm run dev` 本地热更新、`npm run build` 打包到 `dist/`）。
...
├── index.html        # 主程序（孩子端 + 家长端，单文件，同时是 Vite 入口）
├── package.json      # Vite 工程配置与脚本（dev / build / preview）
├── vite.config.js    # Vite 配置（相对 base，便于 Pages 子路径部署）
├── README.md         # 本文件
├── LICENSE           # MIT License
├── .gitignore
├── node_modules/     # 依赖（已忽略，npm install 生成）
├── dist/             # 构建产物（已忽略，npm run build 生成）
└── assets/
    ├── logo.svg      # 项目 Logo
    ├── wechat.svg    # 微信打赏占位图
    └── alipay.svg    # 支付宝打赏占位图
...
**方式一（零工具，最简单）**：直接用浏览器打开 `index.html` 即可（推荐 Chrome / Edge / Safari，平板端体验最佳）。

**方式二（带热更新的开发服务器，推荐开发用）**：需要 Node.js 18+，在项目根目录执行：

```bash
npm install      # 首次安装依赖（Vite）
